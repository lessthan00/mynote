...
command_exists() {
    ...
    DEBIAN_FRONTEND=noninteractive apt-get install -y unzip curl wget git jq openssl
    ;;
...
echo -e "7. Setting up Directories"
	...
	mkdir -p "/etc/dokploy" && mkdir -p "/etc/dokploy/traefik" && mkdir -p "/etc/dokploy/traefik/dynamic" && mkdir -p "/etc/dokploy/logs" && mkdir -p "/etc/dokploy/applications" && mkdir -p "/etc/dokploy/compose" && mkdir -p "/etc/dokploy/ssh" && mkdir -p "/etc/dokploy/traefik/dynamic/certificates" && mkdir -p "/etc/dokploy/monitoring" && mkdir -p "/etc/dokploy/registry"
...
echo -e "10. Setting up Traefik Instance"
		...
		if docker inspect dokploy-traefik > /dev/null 2>&1; then
			...
			docker run -d 				--name dokploy-traefik 				--network dokploy-network 				--restart unless-stopped 				-v /etc/dokploy/traefik/traefik.yml:/etc/traefik/traefik.yml 				-v /etc/dokploy/traefik/dynamic:/etc/dokploy/traefik/dynamic 				-v /var/run/docker.sock:/var/run/docker.sock 				-p 443:443 				-p 80:80 				-p 443:443/udp 				traefik:v$TRAEFIK_VERSION
